...
resource "aws_instance" "loader_instance" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo '/usr/bin/cassandra-stress user profile=./stress.yml n=${var.num_of_ops} cl=local_quorum no-warmup \"ops(insert=1)\" -rate threads=${var.num_threads} fixed=450000/s -mode native cql3 user=scylla password=pass -log file=populating.log  -node ${local.scylla_ips}' > start.sh",
